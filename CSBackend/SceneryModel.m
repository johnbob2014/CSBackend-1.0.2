...
-(NSString *)createMainJSONFile{
    ...
    if (data && !error) {
        ...
        NSURL *fileLocalURL=[[SceneryModel saveJSONDirectoryURL] URLByAppendingPathComponent:ChinaSceneryDataFileName];
        
        filePath=[fileLocalURL path];
        
        if (![data writeToFile:filePath atomically:YES]) {
            NSLog(@"MainJSON写入文件失败！");
            filePath=nil;
        }
...
-(NSString *)createCSceneryJSONFile:(CScenery *)cScenery{
    ...
    [sceneryDic setValue:cScenery.provinceName forKey:[kProvinceName copy]];
    [sceneryDic setValue:cScenery.updateUnix forKey:[kSceneryUpdateUnix copy]];
    ...
    NSError *error;
    NSData *data=[NSJSONSerialization dataWithJSONObject:sceneryDic options:NSJSONWritingPrettyPrinted error:&error];
    if (data && !error) {
        
        NSString *sceneryJSONFileName=[cScenery.name stringByAppendingString:@".json"];
        NSURL *fileLocalURL=[[SceneryModel saveJSONDirectoryURL] URLByAppendingPathComponent:sceneryJSONFileName];
        
        filePath=[fileLocalURL path];
        
        if (![data writeToFile:filePath atomically:YES]) {
            NSLog(@"CSceneryJSON写入文件失败！");
            filePath=nil;
        }
...
+(NSURL *)saveJSONDirectoryURL{
    NSDateFormatter *dateFormatter=[NSDateFormatter new];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    NSString *dateNow=[dateFormatter stringFromDate:[NSDate date]];
    
    NSURL *documentDirectory=[[[NSFileManager defaultManager] URLsForDirectory:NSDocumentDirectory inDomains:NSUserDomainMask]lastObject];
    NSURL *saveJSONDirectoryURL=[documentDirectory URLByAppendingPathComponent:[@"CSBackend_" stringByAppendingString:dateNow]];
    
    //BOOL isDir=YES;
    if (![[NSFileManager defaultManager] fileExistsAtPath:[saveJSONDirectoryURL path]]) {
        [[NSFileManager defaultManager] createDirectoryAtURL:saveJSONDirectoryURL withIntermediateDirectories:YES attributes:nil error:NULL];
    }
    
    return saveJSONDirectoryURL;
}
    ...
    for (NSDictionary *provinceDic in self.provinceArray) {
        ...
        if (provinceName) {
            ...
            for (NSDictionary *sceneryDic in sceneryArray) {
                ...
                CScenery *cScenery=[CScenery initWithName:sceneryName inProvince:cProvince inManagedObjectContext:self.managedObjectContext];
                NSLog(@"%@-%@",cProvince.name,cScenery.name);
                //                dispatch_async(low_Priority_Queue, ^{
                //                    [self refreshCScenery:cScenery.name];
                //                });
            }//end sceneryArray
        }
    }//end provinceArray
...
-(void)setRefreshInterval:(NSTimeInterval)refreshInterval{
    ...
    if (refreshInterval) {
        self.mainTimer=nil;
        self.mainTimer=[NSTimer scheduledTimerWithTimeInterval:refreshInterval target:self selector:@selector(startRefresh) userInfo:nil repeats:YES];
    }else{
        self.mainTimer=nil;
    }
}
